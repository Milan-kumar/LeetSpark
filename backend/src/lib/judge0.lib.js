...
export const pollBatchResults = async (tokens) => {
    while (true) {
        const { data } = await axios.get(`${process.env.JUDGE0_API_URL}/submissions/bacth`, {
            params: {
                tokens: tokens.join(","),
...
export const submitBatch = async (submissions) => {
    ...
    const { data } = await axios.post(`${process.env.JUDGE0.API_URL}/submissions/batch?base64_encoded=false`, { submissions });
